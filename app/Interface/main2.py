...
class Application():
    def __init__(self, master, *args, **kwargs):
        ...
        self.date_l = Label(self.right, text="Today's Date: " + str(date), font=('arial 16 bold'), bg='gray',
                            fg='white')
    ...
    def ajax(self, *args, **kwargs):
        ...
        self.mycursor.execute("SELECT * FROM inventory WHERE id= %s", &  # 91;self.get_id])
        self.pc = self.mycursor.fetchall()
        if self.pc:
            for
        self.r in self.pc:
        self.get_id = self.r &  # 91;0]
                      self.get_name = self.r &  # 91;1]
                                      self.get_price = self.r &  # 91;3]
                                                       self.get_stock = self.r &  # 91;2]
                                                                        self.productname.configure(
                                                                            text="Product's Name: " + str(
                                                                                self.get_name), fg='white', bg='black')
    ...
    def add_to_cart(self, *args, **kwargs):
        ...
        if self.quantity_value > int(self.get_stock):
            ...
            for self.p in products_list:
                self.tempname = Label(self.right, text=str(
                    products_list &  # 91;self.counter]),font=('arial 18 bold'),bg='gray',fg='white')
                    self.tempname.place(x=0, y=self.y_index)
                self.tempqt = Label(self.right, text=str(
                    product_quantity &  # 91;self.counter]), font=('arial 18 bold'), bg='gray', fg='white')
                    self.tempqt.place(x=300, y=self.y_index)
                self.tempprice = Label(self.right, text=str(
                    product_price &  # 91;self.counter]), font=('arial 18 bold'), bg='gray', fg='white')
    ...
    def generate_bill(self, *args, **kwargs):
        ...
        for i in products_list:
            for
        r in self.pc:
        self.old_stock = r &  # 91;2]
                         self.new_stock = int(self.old_stock) - int(self.quantity_value)
        # updating the stock
        self.mycursor.execute("UPDATE inventory SET stock=%s WHERE id=%s", &  # 91;self.new_stock,self.get_id])
        self.conn.commit()

        # inster into transcation
        self.mycursor.execute(
            "INSERT INTO transaction (product_name,quantity,amount,date) VALUES(%s,%s,%s,%s)", &  # 91;self.get_name,self.quantity_value,self.get_price,date])
        self.conn.commit()
        print("Decreased")
